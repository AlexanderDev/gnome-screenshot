...
button_clicked (GtkWidget *button, ScreenshotDialog *dialog)
{
  ...
  if(button == dialog->send_to_telegram)
    res = SCREENSHOT_RESPONSE_SEND_TELEGRAM;
		       ...
		       gpointer               user_data)
{
  ...
  // Send to telegram code
  dialog->send_to_telegram = GTK_WIDGET (gtk_builder_get_object (ui, "send_to_telegram"));
  g_signal_connect (dialog->send_to_telegram, "clicked", G_CALLBACK (button_clicked), dialog);
